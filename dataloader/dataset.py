...
def load_eth3d_data(data_path, current_file):
    A = current_file
    filename = data_path  + current_file[0: len(current_file) - 1]
    left  =Image.open(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'im1.png'
    right = Image.open(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'disp0GT.pfm'
    disp_left, height, width = readPFM(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'disp0GT.pfm'
    disp_right, height, width = readPFM(filename)
    size = np.shape(left)
...
def load_middlebury_data(data_path, current_file):
    A = current_file
    filename = data_path  + current_file[0: len(current_file) - 1]
    left  =Image.open(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'im1.png'
    right = Image.open(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'disp0.pfm'
    disp_left, height, width = readPFM(filename)
    filename = data_path + current_file[0: len(current_file) - 8] + 'disp1.pfm'
    disp_right, height, width = readPFM(filename)
    size = np.shape(left)
    height = size[0]
    width = size[1]
...
class DatasetFromList(data.Dataset): 
    def __init__(self, data_path, file_list, crop_size=[256, 256], training=True, left_right=False, kitti=False, kitti2015=False, eth3d=False, middlebury=False, shift=0):
