...
class GANet(nn.Module):
    def __init__(self, maxdisp=192):
        ...
        # self.conv_x = BasicConv(96, 32, kernel_size=3, padding=1)
        # self.conv_y = BasicConv(96, 32, kernel_size=3, padding=1)
    
        self.conv_refine = nn.Conv2d(32, 32, (3, 3), (1, 1), (1,1), bias=False)
        self.conv_refine_6 = nn.Conv2d(48, 32, (3, 3), (1, 1), (1,1), bias=False)
        self.conv_refine_12 = nn.Conv2d(64, 32, (3, 3), (1, 1), (1,1), bias=False)
        self.conv_refine_24 = nn.Conv2d(96, 32, (3, 3), (1, 1), (1,1), bias=False)
        self.conv_refine_48 = nn.Conv2d(128, 32, (3, 3), (1, 1), (1,1), bias=False)
    ...
    def conv_refine_multi(self, x, shape):
        
        x1 = self.conv_refine(x)
        x1 = F.interpolate(x1, [x1.size()[2]*3,x1.size()[3]*3], mode='bilinear', align_corners=False)
        
        # if x1.shape[2] == int(shape[2]/3):    
        #     x1 = F.interpolate(x1, [x1.size()[2]*3,x1.size()[3]*3], mode='bilinear', align_corners=False)
        # elif x1.shape[2] == int(shape[2]/6):
        #     x1 = F.interpolate(x1, [x1.size()[2]*6,x1.size()[3]*6], mode='bilinear', align_corners=False)
        # elif x1.shape[2] == int(shape[2]/12):
        #     x1 = F.interpolate(x1, [x1.size()[2]*12,x1.size()[3]*12], mode='bilinear', align_corners=False)
        # elif x1.shape[2] == int(shape[2]/24):
        #     x1 = F.interpolate(x1, [x1.size()[2]*24,x1.size()[3]*24], mode='bilinear', align_corners=False)
        # elif x1.shape[2] == int(shape[2]/48):
        #     x1 = F.interpolate(x1, [x1.size()[2]*48,x1.size()[3]*48], mode='bilinear', align_corners=False)
        

        return x1

    def ga_head(self, g, x ,y):
        rem = x
        x = self.cv(x,y)
        x1 = self.conv_refine_multi(rem, g.size())
    ...
    def disp_downsample(self, disp, s_fac):
        ...
        disp = F.interpolate(disp, size=[int((disp.size()[2]/s_fac)), int((disp.size()[3]/s_fac))], mode='bilinear', align_corners=False)
        disp = disp.squeeze(1)
        disp = disp/s_fac
        return disp
    ...
    def getGrid(self, x):
        # print(t[1].size())
        # exit(0)
        # h,w = torch.unbind(x)[0]
        # print('size of meshgrid: ', y.size())
        return bg.cuda(), hg.cuda(), wg.cuda()
        # return y[0], y[1], y[2]
    ...
    def warp(self, x, disp):
        ...
        grid = torch.stack([grid_h.float(), warped_gw], dim=-1)
        grid_normalized = ((grid*2)/torch.Tensor([h,w]).cuda()) - 1
    ...
    def forward(self, x, y):
        ...
        x1 = self.feature(x)
        print("===> x Feature shape: ", x2.shape)
        # exit()
        rem = x1
        x = self.conv_x(x2)

        y1, y2, y3, y4, y5 = self.feature(y)
        print("===> y Feature shape: ", y2.shape)
        y = self.conv_y(y2)

        x = self.cv(x,y)

        print("===> After CV: ", x.shape)

        x1 = self.conv_refine(rem)
        print("===> After Conv_refine: ", x1.shape)
        x1 = F.interpolate(x1, [x1.size()[2]*3,x1.size()[3]*3], mode='bilinear', align_corners=False)
        # x1 = F.interpolate(x1, [x1.size()[2]*6,x1.size()[3]*6], mode='bilinear', align_corners=False)
        x1 = self.bn_relu(x1)
        print("===> Before cat X1: ", x1.shape)
        print("===> Before cat g: ", g.shape)
        g = F.interpolate(g, [x1.size()[2], x1.size()[3]], mode='bilinear', align_corners=False)
        ...
        for i, (x,y) in enumerate(zip([x48, x24, x12, x6, x3], [y48, y24, y12, y6, y3])):
            if i == 0:
                if self.training:
                    disp0, disp1, disp2 = self.ga_head(g, x,y)
                else:
                    disp2 = self.ga_head(g, x,y)
                
            else:
                if self.training:
                    disp_for_warping = self.disp_downsample(disp2, 1.5)
                    y_warped = self.warp(y, disp_for_warping)
                    disp0_res, disp1_res, disp2_res = self.ga_head(g,x,y_warped)
                    disp0 = self.disp_downsample(disp0, 0.5) + disp0_res
                    disp1 = self.disp_downsample(disp1, 0.5) + disp1_res
                    disp2 = self.disp_downsample(disp2, 0.5) + disp2_res
                else:
                    disp_for_warping = self.disp_downsample(disp2, 1.5)
                    y_warped = self.warp(y, disp_for_warping)
                    disp2_res = self.ga_head(g,x,y_warped)
                    disp2 = self.disp_downsample(disp2, 0.5) + disp2_res
