...
#            cost = Variable(torch.FloatTensor(x.size()[0], x.size()[1]*2, self.maxdisp,  x.size()[2],  x.size()[3]).zero_(), volatile= not self.training).cuda()
            ...
            for i in range(self.maxdisp//2 + 1):
                
                if i > 0 :
                    cost[:, :x.size()[1], i+maxdisp//2, :,i:]   = x[:,:,:,i:]
                    # cost[:, :x.size()[1], i+maxdisp//2, :,i:]   = x[:,:,:,i+maxdisp//2:]
                    # cost[:, :x.size()[1], i-maxdisp//2, :,i:]   = x[:,:,:,i-maxdisp//2:]
                    cost[:, :x.size()[1], (maxdisp//2)-i, :,:-i]   = x[:,:,:,:-i]
                    
                    cost[:, x.size()[1]:, i+maxdisp//2, :,i:]   = y[:,:,:,:-i]
                    # cost[:, x.size()[1]:, i-maxdisp//2, :,i:]   = y[:,:,:,:-1*(i-maxdisp//2)]
                    # cost[:, x.size()[1]:, (maxdisp//2)-i, :,i:]   = y[:,:,:,:-1*((maxdisp//2)-i)]
                    cost[:, x.size()[1]:, (maxdisp//2)-i, :,:-i]   = y[:,:,:,i:]
                else:
                    cost[:, :x.size()[1], i + maxdisp//2, :,:]   = x
                    cost[:, x.size()[1]:, i + maxdisp//2, :,:]   = y
...
#        self.disp = Variable(torch.Tensor(np.reshape(np.array(range(self.maxdisp)),[1,self.maxdisp,1,1])).cuda(), requires_grad=False)
    ...
    def forward(self, x):
        assert(x.is_contiguous() == True)
        with torch.cuda.device_of(x):
            disp = Variable(torch.Tensor(np.reshape(np.array(range(-1*(self.maxdisp//2), self.maxdisp//2 + 1)),[1, self.maxdisp, 1, 1])).cuda(), requires_grad=False)
            # if( np.any(x.detach().cpu().numpy() < 0)):
            #             print('**** Negative Value found in disp_weights****')
